# -*- coding: utf-8 -*-
"""
Created on Fri Mar 22 09:49:35 2019
...
print(johnson_lindenstrauss_min_dim(n_samples=1300, eps=[0.99,0.5, 0.1, 0.01]))
print(X.info())
...
def components(K):
    ...
    score=[]
    for i in range(1,K):
        transformer = GaussianRandomProjection(n_components=i,eps=0.05)
        ...
        km = KMeans(n_clusters=2, random_state=0,max_iter=10000,tol=1e-7).fit(X_new)
...
def eps():
    ...
    for i in eps:
        transformer = GaussianRandomProjection(n_components=15,eps=i)
        X_new = transformer.fit_transform(X)
        #label=transformer.predict(X)
...
eps()
#Visualizing the data broken into 3 components
    
transformer = GaussianRandomProjection(n_components=15,eps=0.1)
X_new=transformer.fit_transform(X)
...
def comp1(K):
    ...
    for i in range(1,K):
        print(i)
        agglo=GaussianRandomProjection(n_components=15,eps=0.6)
